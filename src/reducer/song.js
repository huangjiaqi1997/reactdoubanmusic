...
const SONG_GET_SINGLE_SEARCH_RESULT_SUCCESS =
  "SONG_GET_SINGLE_SEARCH_RESULT_SUCCESS";
const SONG_GET_ALBUM_SEARCH_RESULT_SUCCESS =
  "SONG_GET_ALBUM_SEARCH_RESULT_SUCCESS";
...
const error = (data) => ({ data, type: "SONGERROR" });

const getSongUrl = (id, dispatch) => {
  axios.get(`/music/song/url?id=${id}`).then((res) => {
    const { code, data, message } = res.data;
    if (code === 200 && data.length) {
      dispatch(getSongUrlSuccess({ songUrl: data[0].url }));
    } else {
...
export const getHotPlaylist = () => {
  return (dispatch) => {
    axios
      .get(`/music/playlist/track/all?id=3778678&limit=10&offset=1`)
      .then((res) => {
        const { code, data, message } = res.data;
        if (res.data.code === 200) {
          ...
          // 获取歌曲 url
          playlist[0] && getSongUrl(playlist[0].id, dispatch);
          // 获取歌词
          playlist[0] &&
            axios.get(`/music/lyric?id=${playlist[0].id}`).then((res) => {
              if (res.data.code === 200) {
                const lrc = res.data.lrc;
                const lyrics = lrc ? lrc.lyric : "";
                currentSong = { lyrics, ...playlist[0] };

                dispatch(getHotPlaylistSuccess({ playlist, currentSong }));
              } else {
              }
            });
        } else {
...
export const getMyPlaAndCol = (userID) => {
  return (dispatch) => {
    ...
    // 获取用户歌单
    axios.get(`/music/user/playlist?uid=${userID}`).then((res) => {
      if (res.data.code === 200) {
        ...
        let currentSong = {};
        // 成功后获取歌单详情
        axios.get(`/music/playlist/detail?id=${playlistID}`).then((res) => {
          if (res.data.code === 200) {
            ...
            playlist[0] &&
              axios.get(`/music/lyric?id=${playlist[0].id}`).then((res) => {
                if (res.status === 200 && res.data.code === 200) {
                  ...
                  axios
                    .get(`/music/playlist/detail?id=${collectionID}`)
                    .then((res) => {
                      if (res.data.code === 200) {
                        const tracks = res.data.playlist.tracks;
                        for (let i = 0; i < tracks.length; i++) {
...
export const toCurrentSongPlay = (song) => {
  return (dispatch) => {
    dispatch(requestInitiate());
    getSongUrl(song.id, dispatch);
    axios.get(`/music/lyric?id=${song.id}`).then((res) => {
      if (res.status === 200 && res.data.code === 200) {
        const lrc = res.data.lrc;
        const lyrics = lrc ? lrc.lyric : "";
...
export const remove = (song) => {
  return (dispatch, getState) => {
    ...
    axios
      .get(
        `/music/playlist/tracks?op=del&pid=${
          getState().song.playlistID
        }&tracks=${song.id}`
      )
      .then((res) => {
        const { code, data, message } = res.data;
...
export const removePlaylistAll = () => {
  return (dispatch) => dispatch(removePlaylistAllSuccess());
};
export const toggleCollect = (song, isisCollected) => {
  return (dispatch) => {
    // dispatch(requestInitiate());
    axios
      .get(`/music/like?id=${song.id}&like=${!isisCollected}`)
      .then((res) => {
        console.log(res);
        ...
        if (res.status === 200 && res.data.code === 200) {
          ...
          dispatch(error({ message: message ? message : msg }));
        }
      })
      .catch((err) => dispatch(error({ message: "需要登录" })));
...
export const addToPlaylist = (song, string) => {
  return (dispatch, getState) => {
    ...
    axios
      .get(
        `/music/playlist/tracks?op=add&pid=${
          getState().song.playlistID
        }&tracks=${song.id}`
      )
      .then((res) => {
        ...
        if (res.status === 200 && res.data.code === 200) {
          if (!string) {
            ...
            axios.get(`/music/song/detail?ids=${song.id}`).then((res) => {
              if (res.status === 200 && res.data.code === 200) {
...
export const getSingleSearchResult = (keyword) => {
  return (dispatch) => {
    dispatch(requestInitiate());
    axios
      .get(`/music/search?keywords=${keyword}&limit=50`) // &limit 默认30
      .then((res) => {
        if (res.status === 200 && res.data.code === 200) {
          // 无结果时，服务器返回：
          // {
...
export const getAlbumSearchResult = (keyword) => {
  return (dispatch) => {
    dispatch(requestInitiate());
    axios.get(`/music/search?keywords=${keyword}&type=10`).then((res) => {
      if (res.status === 200 && res.data.code === 200) {
        // 无结果时，返回 { code: 200 }
        // result 为 undefined
        const albums = res.data.result ? res.data.result.albums : [];
        const data = [];
...
export const getCommentList = (songID) => {
  return (dispatch) => {
    dispatch(requestInitiate());
    axios.get(`/music/comment/music?id=${songID}&limit=30`).then((res) => {
      if (res.status === 200 && res.data.code === 200) {
...
export const sendComment = (text, id) => {
  return (dispatch) => {
    dispatch(requestInitiate());
    axios
      .get(`/music/comment?t=1&type=0&id=${id}&content=${text}`)
      .then((res) => {
        const { code, data, message } = res.data;
        if (res.status === 200 && res.data.code === 200) {
...
const getSongUrlSuccess = (payload) => ({
  type: "GETSONGURLSUCCESS",
  ...payload,
});
const getMyPlaAndColSuccess = (payload) => ({
  type: SONG_GET_PLA_AND_COL_SUCCESS,
  ...payload,
});
const getHotPlaylistSuccess = (payload) => ({
  type: "GETHOTPLAYLISTSUCCESS",
  ...payload,
});
const getLrcSuccess = (payload) => ({
  type: SONG_GET_LRC_SUCCESS,
  currentSong: payload,
});
const addToPlaylistSuccess = (song) => ({
  song,
...
const toggleCollectSuccess = (song, isisCollected) => ({
  song,
  isisCollected,
  type: SONG_TOGGLE_COLLECT,
});
const getSingleSearchResultSuccess = (payload) => ({
  type: SONG_GET_SINGLE_SEARCH_RESULT_SUCCESS,
  singles: payload,
});
const getAlbumSearchResultSuccess = (payload) => ({
  type: SONG_GET_ALBUM_SEARCH_RESULT_SUCCESS,
  albums: payload,
});
const getCommentListSuccess = (payload) => ({
  type: SONG_GET_COMMENT_LIST_SUCCESS,
  comments: payload.comments,
  hotComments: payload.hotComments,
});
const sendCommentSuccess = (text) => ({
  text,
  type: SONG_SEND_COMMENT_SUCCESS,
});
...
const playReducer = (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case SONG_TOGGLE_COLLECT:
      ...
      // 滤除 来自搜索结果的 并 已收藏的
      if (isisCollected === "") {
        isisCollected = state.collection.some(
          (item) => item.id === action.song.id
        );
        if (isisCollected) {
          msg = "已收藏!";
          return;
    ...
    case SONG_GET_SINGLE_SEARCH_RESULT_SUCCESS:
      return {
        ...
        searchResult: {
          ...
          albums: state.searchResult.albums,
        },
        isLoading: false,
      };
    case SONG_GET_ALBUM_SEARCH_RESULT_SUCCESS:
      return {
        ...
        searchResult: {
          ...
          singles: state.searchResult.singles,
        },
        isLoading: false,
      };
